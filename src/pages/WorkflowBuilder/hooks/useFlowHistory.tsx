...
export function useFlowHistory(
  ...
  const { maxHistoryLength = DEFAULT_MAX_HISTORY, debounceMs = DEFAULT_DEBOUNCE_MS } = options
  ...
  const pendingChangeRef = useRef<{ nodes: Node[]; edges: Edge[] } | null>(null)
  ...
  const commitToHistory = useCallback(() => {
    ...
    const nodesChanged =
      JSON.stringify(newNodes.map((n) => ({ id: n.id, position: n.position, data: n.data }))) !==
      JSON.stringify(
        currentState.nodes.map((n) => ({ id: n.id, position: n.position, data: n.data }))
      )
    const edgesChanged =
      JSON.stringify(newEdges.map((e) => ({ id: e.id, source: e.source, target: e.target }))) !==
      JSON.stringify(
        currentState.edges.map((e) => ({ id: e.id, source: e.source, target: e.target }))
      )
