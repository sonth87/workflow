...
export function PoolNode({ id, data, selected }: NodeProps) {
  ...
  const [label, setLabel] = useState(poolData.label || 'Quy trình đăng ký sử dụng xe của trường')
  ...
  const totalWidth = useMemo(() => lanes.reduce((sum, lane) => sum + lane.size, 0), [lanes])
        ...
        >
          {lanes.map((lane, index) => (
            <div
              ...
              style={{
                width: lane.size,
                borderRight: index < lanes.length - 1 ? '1px solid rgba(0,0,0,0.1)' : 'none',
              }}
            >
              {/* Lane Header */}
              <div
              ...
              >
                {editingLane === lane.id ? (
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === 'Escape') setEditingLane(null)
                    }}
                    autoFocus
