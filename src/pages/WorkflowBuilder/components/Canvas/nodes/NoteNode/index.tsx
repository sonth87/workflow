...
export function NoteNode({ id, data, selected }: NodeProps) {
  ...
  const [content, setContent] = useState(noteData.content || 'Double click to edit note...')
  const [color, setColor] = useState<NoteData['color']>(noteData.color || 'yellow')
  const [fontSize, setFontSize] = useState<NoteData['fontSize']>(noteData.fontSize || 'base')
  const [isEditing, setIsEditing] = useState(false)
  const [showColorPicker, setShowColorPicker] = useState(false)
  const [showFontPicker, setShowFontPicker] = useState(false)

  const updateNodeData = useCallback(
    (newContent: string, newColor?: NoteData['color'], newFontSize?: NoteData['fontSize']) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === id) {
            return {
            ...
            <div className='relative'>
              ...
              {showColorPicker && (
                <div className='absolute top-full left-0 mt-1 flex gap-1 bg-background border rounded shadow-lg p-2 z-50'>
                  {(Object.keys(colorClasses) as Array<keyof typeof colorClasses>).map((c) => (
                    <button
                      key={c}
